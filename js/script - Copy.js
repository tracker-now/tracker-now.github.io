...
function displayRow(i) {
  ...
  $('#detail').append(`<tr id="${trimRonin}"><th scope="row"><a href="#" onclick="reloadScholar('${trimRonin}')"><span class="bi bi-arrow-clockwise"></span></a></th>
    ...
    <td class="schoNext"></td>
    <td class="schoUnclaim"></td>
    <td class="schoSLP"></td>
    <td class="schoManager"></td>
    <td class="schoRonin"></td>
    <td class="schoTotal"></td>
    <td><a href="https://marketplace.axieinfinity.com/profile/${i.eth}/axie/" target="_blank" class="text-primary"><span class="bi bi-link-45deg"></span></a></td>
    <td><a href="#" onclick="editScholar('${trimRonin}')" class="text-primary"><span class="bi bi-pencil-square"></span></a></td>
    <td><a href="#" onclick="removeScholar('${trimRonin}')" class="text-danger"><span class="bi bi-trash-fill"></span></a></td></tr>`);
}
    ...
    if(_config != null) {
      ...
      let data1 = {
          ...
          datasets: [
              {
                  data: _chartData,
                  backgroundColor: ['#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99','#b15928'],
                  borderWidth: 0,
              }
          ]
      }
...
function setOverviewData(slpPrice, totalSLP, totalRoninSLP, totalManagerSLP, totalScholarSLP, totalAvg, count, currency) {
  let dollarUSLocale = Intl.NumberFormat('en-US', {maximumFractionDigits: 2});
  $('#unclaimedTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/locked.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalSLP) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalSLP)} SLP</small></span><small>Unclaimed</small></div></div>`);
  $('#claimedTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/unlocked.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalRoninSLP) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalRoninSLP)} SLP</small></span><small>Ronin account</small></div></div>`);
  $('#managerTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/teacher.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalManagerSLP) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalManagerSLP)} SLP</small></span><small>Manager</small></div></div>`);
  $('#scholarTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/student.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalScholarSLP) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalScholarSLP)} SLP</small></span><small>Scholar</small></div></div>`);
  $('#averageTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/chart.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalAvg/count) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalAvg/count)} SLP</small></span><small>Average SLP</small></div></div>`);
  $('#overAllTotal').html(`<div class="row"><div class="col col-sm-4 ovIcon"><img src="./img/coin.png"></div><div class="col col-sm-8"><span>${dollarUSLocale.format(Math.round(totalSLP+totalRoninSLP) * slpPrice)} ${currency.toUpperCase()}</span><span><small>${Math.round(totalSLP+totalRoninSLP)} SLP</small></span><small>Total</small></div></div>`);
}
